...
import { FileUp, FileText, Printer, Loader2, AlertCircle, CheckCircle2, ChevronLeft, Download, Calendar, FileDown, Table } from 'lucide-react';
...
export default function App() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Use the worker to manipulate the PDF before saving
      const worker = html2pdfModule().set(opt).from(element).toPdf().get('pdf');
      
      await (worker.then((pdf: any) => {
        // Remove extra pages if they exceed the number of items
        while (pdf.internal.getNumberOfPages() > orderData.items.length) {
  ...
  const handleDownloadExcel = () => {
    ...
    const data = items.map(item => ({
      ...
      'Quantidade': formatQty(item.quantity),
